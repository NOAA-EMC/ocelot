...
class Batch(dict):
    ...
    def clone(self):
        return Batch({k: SimpleNamespace(**{name: value.clone() if torch.is_tensor(value) else copy.deepcopy(value)
                                           for name, value in vars(node).items()}) for k, node in self.items()})
