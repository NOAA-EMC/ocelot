...
if [[ "$mode" == smoke ]]; then
    # The bin at the end boundary holds only an instant of target data, so its cycle is
    # always excluded for coverage; ask for one more day than the pairs needed.
    months=(jul2025)
    starts=(2025-07-01T00:00:00)
    ends=(2025-07-03T00:00:00)
fi
