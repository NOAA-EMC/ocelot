...
#SBATCH --mem=250G
#SBATCH -J fsoi_surface_obs
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --time=10:30:00
#SBATCH --output=fsoi_surface_obs_%j.out
#SBATCH --error=fsoi_surface_obs_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL

# ========================================================================
# FSOI: Surface Observations Impact Study
# ========================================================================
# Objective: Determine how ALL observation types impact surface obs
#            forecast error: 2-m T, specific humidity, 10-m u/v wind,
#            surface pressure.
#
# Differences from radiosonde run:
#   - target_instruments: ["surface_obs"]
#   - stratify_by_pressure: false  (surface has no vertical structure)
#   - use_area_weights: true       (obs are geographically uneven)
#   - finite_difference_check: true (10 samples, saved to evaluation/)
# ========================================================================
...
echo "=================================================="
echo "FSOI: Surface Observations Impact Analysis"
echo "=================================================="
...
if [ -z "$GNN_MODEL_DIR_RESOLVED" ]; then
    ...
    echo "  Or: GNN_MODEL_DIR=/path/to/ocelot/gnn_model sbatch $(basename "$0") checkpoints/"
    exit 1
fi
...
# Configuration
CONFIG_FILE="${CONFIG_FILE:-FSOI/configs/fsoi_config_surface_obs.yaml}"
...
OUTPUT_DIR="${FSOI_OUTPUT_DIR:-FSOI/fsoi_outputs/surface_obs_impact_${DATE_TAG}}"
...
python FSOI/test_fsoi.py --checkpoint "$CHECKPOINT_PATH" \
    2>&1 | tee "${LOG_DIR}/test_fsoi_surface_obs_${SLURM_JOB_ID}.log"
...
# ========================================================================
echo "=========================================="
echo "Step 2: Computing FSOI (surface obs targets)..."
echo "=========================================="
python FSOI/fsoi_inference.py \
    ...
    --diagnostics \
    2>&1 | tee "${LOG_DIR}/fsoi_surface_obs_${SLURM_JOB_ID}.log"
...
python FSOI/visualize_fsoi.py \
    ...
    2>&1 | tee "${LOG_DIR}/visualize_surface_obs_${SLURM_JOB_ID}.log" || true

python FSOI/evaluate_fsoi_results.py \
    --input "$CSV_DIR" \
    --output "$OUTPUT_DIR/evaluation" \
    2>&1 | tee "${LOG_DIR}/evaluate_surface_obs_${SLURM_JOB_ID}.log" || true
...
if [ -f "$CSV_DIR/scatter_samples.csv" ]; then
    python FSOI/plot_fsoi_maps.py \
        ...
        2>&1 | tee "${LOG_DIR}/plot_maps_surface_obs_${SLURM_JOB_ID}.log" || true
fi
...
echo "=================================================="
echo "FSOI surface-obs run finished: $(date)"
