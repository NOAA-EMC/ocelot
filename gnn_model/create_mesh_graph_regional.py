...
Main API:
    create_regional_mesh(xy, hierarchical, mesh_splits=None, plot=False)
        Build multi-resolution 2D mesh from projected grid coordinates.
        Returns a dict with keys: m2m_graphs, mesh_pos, G_bottom_mesh,
        all_mesh_nodes.

    create_regional_mesh_from_corners(lon_min, lon_max, lat_min, lat_max, ...)
        Same as create_regional_mesh, but builds the (x, y) meshgrid from a
        domain bounding box given in degrees.

    obs_mesh_conn_regional(obs_xy, G_bottom_mesh, cutoff_factor, num_neighbors, o2m=True)
    ...
    create_regional_mesh_structure(...)
        Adapter that calls create_regional_mesh_from_corners and repackages
        the result into the same dict shape create_mesh() (global) returns,
        so downstream mesh-buffer registration code doesn't need to know
        which geometry it's dealing with.
"""
import matplotlib
...
def _from_networkx_offset(nx_graph, start_index):
    ...
    pyg_graph.edge_index = pyg_graph.edge_index + start_index
    return pyg_graph
...
def _create_flat_mesh(G, nx_stride, plot):
    G_tot = G[0]
...
def _create_hierarchical_mesh(G, mesh_levels, plot):
...
def create_regional_mesh(xy, hierarchical, mesh_splits=None, plot=False):
    """
    ...
    Args:
        ...
        hierarchical: build a hierarchical mesh (with up/down connections)
            instead of a single flat merged mesh.
        mesh_splits: max mesh levels to keep (None = use all available).
        plot: whether to plot diagnostics during creation.
    ...
    mesh_levels = nlev - 1
    if mesh_splits is not None:
        mesh_levels = min(mesh_levels, mesh_splits)

    print(f"[regional mesh] nlev={nlev}, nleaf={nleaf}, mesh_levels={mesh_levels}")
    ...
    if hierarchical:
        return _create_hierarchical_mesh(G, mesh_levels, plot)
    else:
        return _create_flat_mesh(G, NX, plot)
...
def create_regional_mesh_from_corners(lon_min, lon_max, lat_min, lat_max, hierarchical,
    ...
    Args:
        ...
        hierarchical, mesh_splits, plot: same as create_regional_mesh.
            Grid shape is set to 3**(mesh_splits+1) so the full hierarchy fits
            (mesh_splits defaults to 2 if not given).

    Returns: same dict as create_regional_mesh.
    """
    NX = 3
    levels = mesh_splits if mesh_splits is not None else 2
    ...
    return create_regional_mesh([X, Y], hierarchical, mesh_splits=mesh_splits, plot=plot)
...
def obs_mesh_conn_regional(obs_xy, G_bottom_mesh, cutoff_factor=0.67, num_neighbors=4, o2m=True):
    """
    ...
    Args:
        ...
        cutoff_factor: radius (as a multiple of mesh spacing) for the initial
            obs search around each mesh node
        num_neighbors: fallback k-NN count when no obs fall within the radius
        o2m: if True build obs->mesh edges; if False build mesh->obs edges

    Returns:
        edge_index: LongTensor [2, E]
        edge_attr:  FloatTensor [E, 4]  = [dist/dm, dx/dm, dy/dm, 0]
            The 4th feature is zero (no z-component in 2D Cartesian; kept for
            API parity with the global model's 4-dim edge_attr).
    """
...
# ---------------------------------------------------------------------------
# Adapter: repackage into create_mesh()-compatible dict
# ---------------------------------------------------------------------------
...
def create_regional_mesh_structure(lon_min, lon_max, lat_min, lat_max, hierarchical,
    ...
    (mesh buffer registration in GNNLightning, data["mesh"].x/.pos in
    GNNDataModule) can consume either geometry without branching.

    Extra keys not present in the global dict:
        geometry        "regional"
        G_bottom_mesh   networkx DiGraph, needed by obs_mesh_conn_regional
        cutoff_factor, num_neighbors  passed through to obs_mesh_conn_regional
            by the datamodule's connectivity dispatch

    Node/edge features are 2D (x, y), not the global model's 4-dim spherical
    spatial features. Positions are normalized to [-1, 1] using the global
    min/max across all levels combined, since raw Lambert Conformal
    coordinates are in metres (O(1e6)) and overflow fp16.
    """
    regional = create_regional_mesh_from_corners(
        lon_min, lon_max, lat_min, lat_max, hierarchical, mesh_splits=mesh_splits, plot=plot,
    )

    m2m_graphs = regional["m2m_graphs"]

    all_pos = torch.cat([g.pos for g in m2m_graphs], dim=0).to(DEFAULT_DTYPE)
    pos_min = all_pos.min(0).values
    pos_max = all_pos.max(0).values
    pos_scale = (pos_max - pos_min).clamp(min=1e-8)

    def _normalize(pos):
        return (2.0 * (pos.to(DEFAULT_DTYPE) - pos_min) / pos_scale - 1.0)

    mesh_features_torch = [_normalize(g.pos) for g in m2m_graphs]
    mesh_lat_lon_torch = mesh_features_torch  # not real lat/lon; kept for API parity
    mesh_lat_lon_list = [pos.cpu().numpy() for pos in mesh_features_torch]

    # Each level's mesh graph is built bidirectionally (both (u,v) and (v,u)
    # are present), unlike the global mesh's faces_to_edges() which yields
    # one direction per edge and relies on callers (e.g. GNNDataModule) to
    # add the reverse. Keep only one direction here so downstream code that
    # symmetrizes m2m_edge_index_torch doesn't end up double-counting edges.
    m2m_edge_index_torch = []
    m2m_features_torch = []
    for g in m2m_graphs:
        keep = g.edge_index[0] < g.edge_index[1]
        m2m_edge_index_torch.append(g.edge_index[:, keep])
        m2m_features_torch.append(g.edge_attr[keep].to(DEFAULT_DTYPE))

    mesh_structure = {
        "geometry": "regional",
        "G_bottom_mesh": regional["G_bottom_mesh"],
        "cutoff_factor": cutoff_factor,
        "num_neighbors": num_neighbors,
        "m2m_graphs": m2m_graphs,
        "mesh_lat_lon_list": mesh_lat_lon_list,
        "mesh_list": m2m_graphs,
        "m2m_edge_index_torch": m2m_edge_index_torch,
        "m2m_features_torch": m2m_features_torch,
        "mesh_features_torch": mesh_features_torch,
        "mesh_lat_lon_torch": mesh_lat_lon_torch,
    }

    if "mesh_up_graphs" in regional:
        mesh_structure["mesh_up_ei_list"] = [g.edge_index for g in regional["mesh_up_graphs"]]
        mesh_structure["mesh_down_ei_list"] = [g.edge_index for g in regional["mesh_down_graphs"]]
        mesh_structure["mesh_up_features_list"] = [g.edge_attr.to(DEFAULT_DTYPE) for g in regional["mesh_up_graphs"]]
        mesh_structure["mesh_down_features_list"] = [g.edge_attr.to(DEFAULT_DTYPE) for g in regional["mesh_down_graphs"]]
