...
RUN_NAME="${RUN_NAME:-test2-12-12-3-lrfix}"
echo "RUN_NAME=$RUN_NAME"

# Loss/optimizer settings
LOSS_TYPE="${LOSS_TYPE:-mse}"

LR="${LR:-5e-4}"  # 1.5e-4 for 10 years
# LR=1e-4  #finetune purpose
...
LOAD_WEIGHTS_ONLY="${LOAD_WEIGHTS_ONLY:-0}"  # set to 1 for finetune
...
srun --export=ALL --kill-on-bad-exit=1 --cpu-bind=cores python train_gnn.py \
	...
	--val_start_date 2024-11-01 \
	--val_end_date 2025-01-01 \
	--train_window_days 12 \
	--val_window_days 8 \
	--val_mode sequential \
	--val_stride_days 8 \
	--val_update_every_n_epochs 800 \
	--lr "$LR" \
