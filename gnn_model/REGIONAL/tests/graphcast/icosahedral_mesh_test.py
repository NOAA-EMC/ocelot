...
import chex
from ocelot.graphcast import icosahedral_mesh
import numpy as np
...
class IcosahedralMeshTest(unittest.TestCase):
    ...
    def test_get_hierarchy_of_triangular_meshes_for_sphere(self):
        for splits in range(5):
            with self.subTest(splits=splits):
                ...
                for mesh_i, mesh in enumerate(meshes):
                    ...
                    num_expected_vertices, num_expected_faces = _get_mesh_spec(
                        mesh_i)
                    _assert_valid_mesh(
                        mesh, num_expected_vertices, num_expected_faces)
                    ...
                    if prev_vertices is not None:
                        ...
                        np.testing.assert_array_equal(
                            leading_mesh_vertices, prev_vertices)

                    # Increase the expected/previous values for the next
                    # iteration.
        ...
        for splits in range(4):
            with self.subTest(splits=splits):
                ...
                expected_faces = np.concatenate(
                    [m.faces for m in mesh_hierarchy], axis=0)
                np.testing.assert_array_equal(
                    mesh.vertices, mesh_hierarchy[-1].vertices)
...
def _assert_positive_face_orientation(vertices, faces):
    ...
    face_orientation /= np.linalg.norm(face_orientation,
    ...
    dot_center_orientation = np.einsum(
        "ik,ik->i", face_orientation, face_centers)
