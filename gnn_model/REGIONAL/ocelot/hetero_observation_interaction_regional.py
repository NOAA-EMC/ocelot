...
class RegionalHeteroObservationGraphModel(HeteroObservationGraphModel):
    ...
    def create_mesh_structures(self) -> None:
        ...
        mesh_structure = create_regional_mesh_from_corners(x_min, x_max, y_min, y_max, self.args)
        ...
        # mesh_graph is not used by setup_mesh() below, but kept for API parity.
    ...
    def setup_mesh(self):
        ...
        mesh_x = torch.cat([g.pos for g in all_graphs], dim=0).to(torch.float32)
        mesh_edge_index = torch.cat([g.edge_index for g in all_graphs], dim=1)
        mesh_edge_attr = torch.cat([g.edge_attr for g in all_graphs], dim=0).to(torch.float32)
        ...
        if self.is_hierarchical:
            ...
            for i, g in enumerate(all_graphs):
                ...
                self.register_buffer(f"mesh_edge_index_level_{i}", g.edge_index.clone())
                self.register_buffer(f"mesh_edge_attr_level_{i}", g.edge_attr.clone().to(torch.float32))
            for i, up_g in enumerate(ms.get("mesh_up_graphs", [])):
                self.register_buffer(f"mesh_up_edge_index_{i}", up_g.edge_index.clone())
                self.register_buffer(f"mesh_up_edge_attr_{i}", up_g.edge_attr.clone().to(torch.float32))
            for i, dn_g in enumerate(ms.get("mesh_down_graphs", [])):
                self.register_buffer(f"mesh_down_edge_index_{i}", dn_g.edge_index.clone())
                self.register_buffer(f"mesh_down_edge_attr_{i}", dn_g.edge_attr.clone().to(torch.float32))
