...
def process_zarr_file(zarr_file, output_parquet_dir, num_cores=8):
    ...
    # Open dataset lazily
    ds = xr.open_zarr(zarr_file, chunks={"time": 5_000_000}, consolidated=False)
    ...
    if not np.issubdtype(ds["time"].dtype, np.datetime64):
        ...
        ds["time"] = pd.to_datetime(ds["time"].values, unit="s", errors="coerce")
    ...
    success = sum(results)
    print(f"🏁 Finished {success}/{len(unique_bins)} partitions successfully for {base_name}")
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python script.py <data_path> <obs_type_pattern> [num_cores]")
    ...
    output_name = obs_type_pattern.replace('*.zarr', '').replace('_*.zarr', '').rstrip('_')
    if not output_name:
    ...
    output_parquet_dir = os.path.join(base_parquet_dir, f"{output_name}.parquet")
    ...
    for i, zarr_file in enumerate(zarr_files, 1):
        ...
        try:
            success, bins = process_zarr_file(zarr_file, output_parquet_dir, num_cores)
