...
def extract_instruments(df: pd.DataFrame) -> List[str]:
    ...
    """
    train_loss_cols = [col for col in df.columns if col.startswith('train_loss_') and col.endswith('_target_epoch')]
    val_loss_cols = [col for col in df.columns if col.startswith('val_loss_') and col.endswith('_target_epoch')]
...
def prepare_plot_data(df: pd.DataFrame, min_epochs: int = 2, max_points: int = 200) -> Tuple[pd.DataFrame, str, str]:
    ...
    if len(df_epoch) >= min_epochs:
        print(f"Using epoch-based plotting ({len(df_epoch)} epochs available)\n")
        return df_epoch, 'Epoch', 'train_loss_epoch'
    else:
        print(f"⚠ Only {len(df_epoch)} epoch(s) available (minimum {min_epochs} required)")
        ...
        # train_loss_epoch is only logged at epoch boundaries, so look for step-level losses
        ...
        # 3. Fall back to 'train_loss_epoch' (will only have epoch boundary points)
        ...
        if 'train_loss_step' in df.columns:
            ...
            print(f"DEBUG: train_loss_step has {per_step_count} non-null values")
            if per_step_count > len(df_epoch) * 2:  # Need significantly more than just epoch boundaries
                loss_col = 'train_loss_step'
                print(f"DEBUG: Using 'train_loss_step' column for step-based plotting ({per_step_count} points)")
        ...
        if loss_col is None and 'train_loss' in df.columns:
            ...
            if per_step_count > len(df_epoch):
                loss_col = 'train_loss'
                print(f"DEBUG: Using 'train_loss' column for step-based plotting ({per_step_count} points)")
        
        if loss_col is None:
            # Look for per-step instrument losses (columns like 'train_loss_satellite_atms_target_step')
            ...
            if step_loss_cols:
                ...
                print(f"DEBUG: Using '{loss_col}' column for step-based plotting ({per_step_count} points)")
                print(f"DEBUG: Found {len(step_loss_cols)} per-step instrument loss columns")
            else:
                # Last resort: use train_loss_epoch (will only have 2 points)
                loss_col = 'train_loss_epoch'
                print(f"DEBUG: WARNING - Using 'train_loss_epoch' column (only has {df[loss_col].notna().sum()} points)")
                print(f"DEBUG: This will result in very few data points. Consider logging per-step losses.")
        ...
        print(f"DEBUG: Rows with {loss_col}.notna(): {df[loss_col].notna().sum()}")
        
        df_steps = df[(df['step'].notna()) & (df[loss_col].notna())].copy().reset_index(drop=True)
        
        print(f"DEBUG: After filtering for step.notna() & {loss_col}.notna(), df_steps has {len(df_steps)} rows")
        ...
        if len(df_steps) <= 2:
            ...
            print(f"  Non-null {loss_col} in original df: {df[loss_col].notna().sum()}")
            print(f"  Sample of non-null {loss_col} rows:")
            sample_df = df[df[loss_col].notna()][['step', 'epoch', loss_col]].head(10)
            print(sample_df.to_string())
        if len(df_steps) > 0:
            print(f"DEBUG: First few step values: {df_steps['step'].head(10).tolist()}")
            print(f"DEBUG: Last few step values: {df_steps['step'].tail(10).tolist()}")
            print(f"DEBUG: Step range: {df_steps['step'].min()} to {df_steps['step'].max()}")
        ...
        if len(df_steps) > max_points:
            ...
            print(f"Downsampled from {original_len} to {len(df_steps)} points for plotting")
            print(f"DEBUG: After downsampling, step values (first 10): {df_steps['step'].head(10).tolist()}")
            print(f"DEBUG: After downsampling, step values (last 10): {df_steps['step'].tail(10).tolist()}\n")
        else:
            print(f"Using all {len(df_steps)} training steps (no downsampling needed)\n")
        
        return df_steps, 'Step', loss_col


def plot_overall_loss(df_data: pd.DataFrame, plot_dir: str, exp_name: str, x_axis: str = 'Epoch', loss_col: str = 'train_loss_epoch') -> None:
    ...
    print(f"  DEBUG: {loss_col} values (first 10): {df_data[loss_col].head(10).tolist()}")
    print(f"  DEBUG: Number of non-null {loss_col}: {df_data[loss_col].notna().sum()}")
    ...
    if val_col in df_data.columns:
        ...
        if len(df_val) > 0:
            ax.plot(df_val[x_col].astype(float), df_val[val_col].astype(float), 'r-',
                    label='Validation Loss', linewidth=2, marker='s', markersize=5)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(plot_dir, 'overall_loss.png'), dpi=150, bbox_inches='tight')
    plt.close()
...
def plot_learning_rate(df: pd.DataFrame, plot_dir: str, exp_name: str) -> None:
    ...
    if 'lr' in df.columns and 'step' in df.columns:
        ...
        if len(lr_data) > 0:
            print(f"  DEBUG: LR step range: {lr_data['step'].min()} to {lr_data['step'].max()}")
            print(f"  DEBUG: LR value range: {lr_data['lr'].min():.2e} to {lr_data['lr'].max():.2e}")
            ...
            if len(lr_data) > 50:
                ...
                ax.plot(x_lr, y_lr, 'g-', linewidth=2, marker='o', markersize=4)
            
            ax.set_xlabel('Training Step', fontsize=12)
            ax.set_ylabel('Learning Rate', fontsize=12)
            ax.set_title(f'Learning Rate Schedule - {exp_name}', fontsize=14, fontweight='bold')
            ax.grid(True, alpha=0.3)
            ax.ticklabel_format(style='scientific', axis='y', scilimits=(0,0))
            plt.tight_layout()
            plt.savefig(os.path.join(plot_dir, 'learning_rate.png'), dpi=150, bbox_inches='tight')
            plt.close()
...
def plot_train_loss_by_instrument(df_data: pd.DataFrame, instruments: List[str], 
                                   plot_dir: str, exp_name: str, x_axis: str = 'Epoch') -> None:
    ...
    for instrument in instruments:
        ...
        if train_col in df_data.columns:
            ...
            if len(df_line) > 0:
                ax.plot(df_line[x_col].astype(float), df_line[train_col].astype(float),
                        label=instrument, linewidth=2, marker='o', markersize=4)
    
    ax.set_xlabel(x_axis, fontsize=12)
    ax.set_ylabel('Training Loss', fontsize=12)
    ax.set_title(f'Training Loss by Instrument - {exp_name}', fontsize=14, fontweight='bold')
    ax.legend(fontsize=9, loc='best')
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(plot_dir, 'train_loss_by_instrument.png'), dpi=150, bbox_inches='tight')
    plt.close()
    print(f"  ✓ Saved: train_loss_by_instrument.png")


def plot_val_loss_by_instrument(df_data: pd.DataFrame, instruments: List[str], 
                                 plot_dir: str, exp_name: str, x_axis: str = 'Epoch') -> None:
    ...
    for instrument in instruments:
        ...
        if val_col in df_data.columns:
            ...
            if len(df_line) > 0:
                has_val_data = True
                ax.plot(df_line[x_col].astype(float), df_line[val_col].astype(float),
                        label=instrument, linewidth=2, marker='o', markersize=4)
    
    if has_val_data:
        ax.set_xlabel(x_axis, fontsize=12)
        ax.set_ylabel('Validation Loss', fontsize=12)
        ax.set_title(f'Validation Loss by Instrument - {exp_name}', fontsize=14, fontweight='bold')
        ax.legend(fontsize=9, loc='best')
        ax.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(plot_dir, 'val_loss_by_instrument.png'), dpi=150, bbox_inches='tight')
        plt.close()
    ...
    for instrument in instruments:
        ...
        if train_col in df_data.columns or val_col in df_data.columns:
            ...
            if train_col in df_data.columns:
                ...
                if len(df_train) > 0:
                    ax.plot(df_train[x_col].astype(float), df_train[train_col].astype(float), 'b-',
                            label='Training', linewidth=2, marker='o', markersize=5)
            ...
            if val_col in df_data.columns:
                ...
                if len(df_val) > 0:
                    ax.plot(df_val[x_col].astype(float), df_val[val_col].astype(float), 'r-',
                            label='Validation', linewidth=2, marker='s', markersize=5)
...
def plot_training_metrics(metrics_csv_path, output_dir='plots', exp_name='training_metrics', version='version_0', min_epochs=4):
    """
    ...
    plot_train_loss_by_instrument(df_plot, instruments, plot_dir, exp_name, x_axis)
    plot_val_loss_by_instrument(df_plot, instruments, plot_dir, exp_name, x_axis)
  ...
  python -m ocelot.plot_training_metrics \\
    ...
    if args.metrics_csv:
        ...
        if args.version:
            ...
            print(f"Found {len(version_dirs)} version(s), using latest: {version_name}")
    ...
    if not os.path.exists(metrics_csv_path):
        ...
        if not args.metrics_csv:
            print(f"Hint: Check if experiment '{args.exp_name}' exists in {args.base_dir}")
        return 1
    
    # Generate plots
    try:
        plot_training_metrics(metrics_csv_path, args.output_dir, args.exp_name, version=version_name, min_epochs=args.min_epochs)
        return 0
