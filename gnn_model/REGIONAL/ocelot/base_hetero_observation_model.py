...
def _validate_graph(data: 'HeteroData',
    ...
    for (src, _, dst), edge_index in data.edge_index_dict.items():
        ...
        if src == "mesh":
            ...
            # Target nodes might not have input 'x', but they have a defined
            # number of nodes
        ...
        if dst == "mesh":
            ...
            # This case should not typically happen, but handle for
            # completeness
        ...
        print(
            f"[CHECK] Edge {src}->{dst} ok: ({edge_index[0].min()}–{edge_index[0].max()}) to ({edge_index[1].min()}–{edge_index[1].max()})")
...
class BaseHeteroGraphModel(ARDOPModel):
    ...
    def __init__(self, args):
        ...
        self.args = args
        self.debug_gradients = args.debug_gradients if hasattr(
            args, 'debug_gradients') else False
        ...
        # GNNs: Encodes obs features onto mesh
        self.observation_encoders = nn.ModuleDict()
        # GNNs: Decodes mesh features to obs
        self.observation_decoders = nn.ModuleDict()
        # MLPs: Maps hidden state to output dim
        self.output_mappers = nn.ModuleDict()
        # Embeds scan angle for satellite data
        self.scan_angle_embedders = nn.ModuleDict()

        # A dedicated embedder for the static mesh features
        self.mesh_embedder = utils.make_mlp(
            [args.mesh_feature_dim] + self.mlp_blueprint_end)
    ...
    def setup_mesh(self):
        ...
        if getattr(self, 'is_hierarchical', False):
            ...
            for i, down_ei in enumerate(ms['mesh_down_ei_list']):
                self.register_buffer(
                    f'mesh_down_edge_index_{i}', down_ei.clone())
            for i, down_f in enumerate(ms['mesh_down_features_list']):
                self.register_buffer(
                    f'mesh_down_edge_attr_{i}', down_f.clone())
        ...
        try:
            ...
            self.mesh_graph = Data(
                ...
                pos=torch.from_numpy(
                    mesh_structure['mesh_lat_lon_list'][0]).float(),
            ...
            raise FileNotFoundError(
                f"Could not find grid coordinates file: {e}")
    ...
    def decode(
        ...
        def _mem_decode(label):
            if torch.cuda.is_available():
                ...
                logger.debug(
                    f"[MEM] decode {label}: allocated={a:.2f} GiB  reserved={r:.2f} GiB")
        ...
        for edge_type, edge_index in data.edge_index_dict.items():
            ...
            logger.info(
                f"Processing edge_type: {edge_type}, src_type: {src_type}, dst_type: {dst_type}")
            if src_type == 'mesh' and dst_type.endswith('_target'):
                ...
                if self.training:
                    if target_features.shape[0] == 0:
                        ...
                        # to avoid retaining mesh_features and the full
                        # processor graph.
                        decoded_target_features = torch.zeros_like(
                            target_features)
                    else:
                ...
                # Combine decoded mesh state with embedded target context
                # (target .x: meta + aux).
                target_context = data[dst_type].x
                context_embedded = self.scan_angle_embedders[dst_type](
                    target_context)
                final_features = torch.cat(
                    [decoded_target_features, context_embedded], dim=-1)
                ...
                if (
                    ...
                    predictions[dst_type].register_hook(
                        print_grad(f"predictions[{dst_type}]"))
    ...
    def _get_instrument_weight(self, node_type: str) -> float:
        ...
        base = node_type[:-len('_target')
                         ] if node_type.endswith('_target') else node_type
    ...
    def _generic_step(
        ...
        if stage == 'train' and batch_idx == 0:
            ...
            for node_type in batch.node_types:
                ...
                logger.info(f"\nNode Type: {node_type}")
                logger.info(
                    f"  num_nodes: {node_data.num_nodes if hasattr(node_data, 'num_nodes') else 'N/A'}")
        ...
        # C++/Python reference cycles that prevent Python GC from freeing
        # saves.
        ...
        if stage == 'train' and batch_idx == 0:
            ...
            for node_type, pred in predictions.items():
                ...
                if hasattr(
                    ...
                    logger.info(
                        f"  Input Features  | Shape: {tuple(inputs.shape)} | {_stats(inputs)}")
                    logger.info(
                        f"  Target Labels   | Shape: {tuple(targets.shape)} | {_stats(targets)}")
                    logger.info(
                        f"  Predictions     | Shape: {tuple(pred.shape)} | {_stats(pred)}")
                else:
                    missing_attrs = []
                    ...
                    logger.info(
                        f"  (Missing attributes: {', '.join(missing_attrs) if missing_attrs else 'unknown'})")
                logger.info("-" * 80)
            logger.info("=" * 80)
        ...
        for node_type, pred in predictions.items():
            ...
            if hasattr(batch[node_type], 'y'):
                ...
                mask = batch[node_type].valid_mask if hasattr(
                    batch[node_type], 'valid_mask') else None

                # Diagnostic: log valid-mask coverage for surface obs node
                # types
                ...
                # We check for numel on the original prediction tensor before
                # masking.
                if pred.numel() > 0:
                    logger.debug(
                        f"Computing loss for {node_type} | pred: {tuple(pred.shape)}, "
                    ...
                    if grad_weight > 0:
                        ...
                        loss = loss + grad_weight * \
                            gradient_loss(pred, target, grad_edge_index, mask=mask)
                    ...
                    multiscale_weight = float(
                        getattr(
                            ...
                            0.0) or 0.0)
                    if multiscale_weight > 0 and hasattr(
                            batch[node_type], 'pos'):
                        scales = getattr(
                            self.args, 'multiscale_scales', None) or (
                            0.5, 1.0, 2.0, 4.0)
                        loss = loss + multiscale_weight * multiscale_loss(
                            pred, target, batch[node_type].pos, mask=mask, scales=scales
                        )

                    # A NaN loss can occur if the mask is all False, which is
                    # valid.
                    if not torch.isnan(loss):
                        ...
                        # processor saves ~1.28 GiB) alive for the entire
                        # epoch.
                        self.log(
                            f'{stage}_loss_{node_type}',
                            loss.detach(),
                            on_step=True,
                            on_epoch=True,
                            prog_bar=False,
        ...
        if total_loss is None:
            avg_loss = self._zero_loss_with_graph(
                predictions, mesh=_stored_mesh)
        elif total_weight > 0:
        ...
        # and enter file_exists() a different number of times → collective
        # mismatch.
        sync = stage in ('val', 'test')
        self.log(
            f'{stage}_loss',
    ...
    def on_after_backward(self):
        ...
        if self.trainer.global_step < 3:
            ...
            if self.observation_encoders:
                first_encoder_key = next(
                    iter(self.observation_encoders.keys()))
                encoder_weight = self._first_weight(
                    self.observation_encoders[first_encoder_key])
                if encoder_weight is None:
                    print(
                        f"  Encoder ('{first_encoder_key}') | No weight parameter found!")
                elif encoder_weight.grad is not None:
                    grad_mean = encoder_weight.grad.mean()
                    grad_std = encoder_weight.grad.std()
                    print(
                        f"  Encoder ('{first_encoder_key}') | Grad Mean: {grad_mean:.2e}, Grad Std: {grad_std:.2e}")
                else:
                    print(f"  Encoder ('{first_encoder_key}') | Grad is None!")

            # Check gradient of a decoder (last weight found)
            if self.observation_decoders:
                first_decoder_key = next(
                    iter(self.observation_decoders.keys()))
                decoder_weight = self._last_weight(
                    self.observation_decoders[first_decoder_key])
                if decoder_weight is None:
                    print(
                        f"  Decoder ('{first_decoder_key}') | No weight parameter found!")
                elif decoder_weight.grad is not None:
                    grad_mean = decoder_weight.grad.mean()
                    grad_std = decoder_weight.grad.std()
                    print(
                        f"  Decoder ('{first_decoder_key}') | Grad Mean: {grad_mean:.2e}, Grad Std: {grad_std:.2e}")
                else:
    ...
    def _debug_state_gradients(self):
        ...
        _report(f"embedder[{input_key}] first layer",
                embedder[0].weight if embedder is not None else None)

        encoder = _lookup("observation_encoders", encoder_key)
        _report(
            f"encoder[{encoder_key}] first layer",
            self._first_weight(encoder))

        decoder = _lookup("observation_decoders", decoder_key)
        _report(
            f"decoder[{decoder_key}] last layer",
            self._last_weight(decoder))

        scan_embedder = _lookup("scan_angle_embedders", target_key)
        _report(
            f"scan_angle_embedder[{target_key}] first layer",
            scan_embedder[0].weight if scan_embedder is not None else None)

        output_mapper = _lookup("output_mappers", target_key)
        _report(f"output_mapper[{target_key}] last layer",
                output_mapper[-1].weight if output_mapper is not None else None)

    def _log_and_visualize(self,
                           batch: 'HeteroData',
        ...
        if stage == 'val':
            ...
            if should_save:
                ...
                output_dir = os.path.join(
                    'debug_outputs', self.args.exp_name, version)
                ...
                filename = os.path.join(
                    output_dir,
                    f'debug_data_epoch_{current_epoch}_step_{current_step}_rank{rank}.pt')
                print(
                ...
                # Also plot examples only when we save debug data (avoid
                # excessive plotting)
    ...
    def _get_std_for_loss(pred_value_tensor, std_value_representation=None):
        ...
        # For now, assume std_value_representation is already the actual std if
        # not None
