...
class ResampleDataCallback(pl.Callback):
    ...
    def __init__(self, full_start_date, full_end_date, window_days=7):
        ...
        self.full_end_date = pd.to_datetime(full_end_date) - pd.Timedelta(days=window_days)
    ...
    def on_train_epoch_start(self, trainer, pl_module):
        ...
        datamodule.start_date = new_start
        datamodule.end_date = new_end
...
class SequentialDataCallback(pl.Callback):
    ...
    def on_train_epoch_start(self, trainer, pl_module):
        ...
        # Define the window for the current epoch
        new_start = self.current_start_date
        new_end = new_start + self.window_days

        # Ensure the window doesn't exceed the full end date
        ...
        # Re-run the setup logic for the new window
        datamodule.setup("fit")

        # Advance the start date for the next epoch
        self.current_start_date = new_end
