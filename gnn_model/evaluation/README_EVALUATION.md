...
- `evaluation/scripts/run_evaluation.sh` (Slurm batch wrapper)
- `evaluation/scripts/evaluations.py` (OCELOT vs Truth plots + pointwise metrics)
- `evaluation/scripts/compare_to_gfs.py` (build `*_vs_gfs.csv` by interpolating GFS to obs)
- `evaluation/scripts/plot_gfs_compare.py` (single entrypoint: RMSE-vs-fhr + per-fhr maps from `*_vs_gfs.csv`)

It also covers **mesh-grid** comparisons, where OCELOT and GFS are evaluated on the same OCELOT mesh points:

- `evaluation/scripts/compare_mesh_to_gfs.py` (interpolate GFS to OCELOT mesh points)
- `evaluation/scripts/plot_mesh_vs_gfs_maps.py` (maps: OCELOT_on_mesh vs GFS_on_mesh)

The most common workflow is:

1. Generate OCELOT prediction CSVs (training `val_csv/...` or testing `predictions/pred_csv/obs-space/...`).
2. Use `evaluation/scripts/evaluations.py --mode plots` to make OCELOT-vs-Truth figures.
3. (Optional) Use `evaluation/scripts/compare_to_gfs.py` to create `*_vs_gfs.csv`.
4. Use `evaluation/scripts/plot_gfs_compare.py` (or `evaluation/scripts/run_evaluation.sh` with `MODE=gfs_compare`) to plot OCELOT/GFS/Truth comparisons.

If you want a single entrypoint that generates *both* RMSE curves and map panels, use:

- `evaluation/scripts/plot_gfs_compare.py`
...
These are produced by `evaluation/scripts/compare_to_gfs.py` and include (at least):

- `pred_*`, `true_*` (OCELOT and truth)
- `gfs_*` (GFS interpolated to each observation)
- `lead_hours_nominal` and/or `fhr_used`

`evaluation/scripts/plot_gfs_compare.py` expects files like:
...
## 2) `evaluation/scripts/evaluations.py` (OCELOT vs Truth plots + metrics)
...
```bash
conda run -n gnn-env python evaluation/scripts/evaluations.py --mode plots --has_ground_truth \
  --data_dir val_csv/Rand_TenYear_nl16 \
  --plot_dir evaluation/figures/Rand_TenYear_nl16/epoch100/batch0/fhr3 \
...
conda run -n gnn-env python evaluation/scripts/evaluations.py --mode plots --has_ground_truth \
  ...
  --plot_dir evaluation/figures/test/obs/init_2025030100 \
...
for fhr in 3 6 9 12; do
  conda run -n gnn-env python evaluation/scripts/evaluations.py --mode plots --has_ground_truth \
    --data_dir val_csv/Rand_TenYear_nl16 \
    --plot_dir evaluation/figures/Rand_TenYear_nl16/epoch100/batch0/fhr${fhr} \
...
## 3) `evaluation/scripts/compare_to_gfs.py` (build `*_vs_gfs.csv`)
...
## 4) `evaluation/scripts/plot_gfs_compare.py` (RMSE and/or maps vs forecast hour)
...
```bash
conda run -n gnn-env python evaluation/scripts/plot_gfs_compare.py \
  --init_time 2025030100 \
  --data_dir predictions/pred_csv/obs-space \
...
```bash
conda run -n gnn-env python evaluation/scripts/compare_mesh_to_gfs.py \
...
```bash
conda run -n gnn-env python evaluation/scripts/plot_mesh_vs_gfs_maps.py \
  --csv predictions/<experiment>/pred_csv/mesh-grid/<instrument>_init_<YYYYMMDDHH>_f<FFF>_gfs_on_ocelot_mesh.csv \
  --plot_dir evaluation/figures/<experiment>/ocelot_on_mesh_vs_gfs_on_mesh/init_<YYYYMMDDHH>/fhr<HH> \
  --var t2m
```
...
## 6) `evaluation/scripts/run_evaluation.sh` (Slurm wrapper)

`evaluation/scripts/run_evaluation.sh` runs a loop over init times (12-hour increments) and calls either:

- `evaluation/scripts/evaluations.py` (MODE=`standard`) for OCELOT vs Truth plots
- `evaluation/scripts/plot_gfs_compare.py` (MODE=`gfs_compare`) for OCELOT/GFS/Truth RMSE-vs-fhr plots (and optional maps)

Note: `evaluation/scripts/run_evaluation.sh` is for **obs-space** artifacts. For **mesh-grid** plots, use the commands in the “Mesh-grid evaluation” section
or the dedicated end-to-end wrapper `evaluation/scripts/run_pred_eval_gfs.sh`.

### A) Submit with a date range

```bash
sbatch evaluation/scripts/run_evaluation.sh 2025030100 2025030912
```

If you omit args it defaults to a single init time.

### B) Configure what it runs
Open `evaluation/scripts/run_evaluation.sh` and edit the **CONFIGURATION** section:

- `MODE`:
  - `standard` (default): `evaluation/scripts/evaluations.py`
  - `gfs_compare`: `evaluation/scripts/plot_gfs_compare.py`
...
```bash
conda run -n gnn-env python evaluation/scripts/evaluations.py --help
conda run -n gnn-env python evaluation/scripts/plot_gfs_compare.py --help
conda run -n gnn-env python evaluation/scripts/compare_to_gfs.py --help
conda run -n gnn-env python evaluation/scripts/compare_mesh_to_gfs.py --help
conda run -n gnn-env python evaluation/scripts/plot_mesh_vs_gfs_maps.py --help
```
