...
# Plot target
plt.subplot(1, 2, 1)
sc1 = plt.scatter(df["lon_deg"], df["lat_deg"], c=df[f"true_bt_15"], cmap="viridis", s=5)
plt.colorbar(sc1)
plt.title(f"True Brightness Temp - Channel 15")
plt.xlabel("Longitude")
plt.ylabel("Latitude")

# Plot prediction
plt.subplot(1, 2, 2)
sc2 = plt.scatter(df["lon_deg"], df["lat_deg"], c=df[f"pred_bt_15"], cmap="viridis", s=5)
plt.colorbar(sc2)
plt.title(f"Predicted Brightness Temp - Channel 15")
plt.xlabel("Longitude")
plt.ylabel("Latitude")

plt.tight_layout()
plt.savefig(f"bt_map_channel_15.png")
plt.close()